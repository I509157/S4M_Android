...
public class PasscodeActionHandlerImpl implements PasscodeActionHandler {
    ...
    @Override
    public void shouldTryPasscode(final char[] originPasscode, PasscodeInputMode mode, final Fragment fragment) throws PasscodeValidationException {
        ...
        ClientPolicy clientPolicy = clientPolicyManager.getClientPolicy(false);
        PasscodePolicy passcodePolicy = clientPolicy.getPasscodePolicy();
        char[] tempPasscode = Arrays.copyOf(originPasscode, originPasscode.length);
        if(passcodePolicy.isDigitsOnly() && passcodePolicy.isLocalizingDigitsToLatin()) {
            tempPasscode = passcodeToLatinDigits(tempPasscode);
        }
        final char[] passcode = Arrays.copyOf(tempPasscode, tempPasscode.length);
        ...
        switch (mode) {
            ...
            case MATCH:
                ...
                executorService.submit(() -> {
                    ClientPolicy refreshedClientPolicy = clientPolicyManager.getClientPolicy(true);
                    PasscodePolicy refreshedPasscodePolicy = refreshedClientPolicy.getPasscodePolicy();
                    boolean isLogPolicyEnabled = refreshedClientPolicy.isLogEnabled();
                    clientPolicyManager.initializeLoggingWithPolicy(isLogPolicyEnabled);
                    if (!refreshedPasscodePolicy.allowsFingerprint() &&
                            secureStoreManager.getApplicationStoreState() == EncryptionState.PASSCODE_BIOMETRIC) {
    ...
    private char[] passcodeToLatinDigits(char[] passcode) {
