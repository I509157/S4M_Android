...
import com.sap.support.databinding.FragmentIncidentinfosetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing IncidentInfo entity.
 * This fragment is contained in the {@link IncidentInfoSetActivity}.
 */
public class IncidentInfoSetCreateFragment extends InterfacedFragment<IncidentInfo> {

    private static final Logger LOGGER = LoggerFactory.getLogger(IncidentInfoSetCreateFragment.class);
    ...
    /** IncidentInfo object and it's copy: the modifications are done on the copied object. */
    private IncidentInfo incidentInfoEntity;
    private IncidentInfo incidentInfoEntityCopy;

    /** DataBinding generated class */
    private FragmentIncidentinfosetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** IncidentInfo ViewModel */
    ...
    /**
     * This fragment is used for both update and create for IncidentInfoSet to enter values for the properties.
     ...
     *            IncidentInfo if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.incidentInfo.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.incidentInfo.getLocalName();
        }

        ((IncidentInfoSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            incidentInfoEntity = createIncidentInfo();
        } else {
            incidentInfoEntity = viewModel.getSelectedEntity().getValue();
        }

        IncidentInfo workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (IncidentInfo)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            incidentInfoEntityCopy = (IncidentInfo) incidentInfoEntity.copy();
            incidentInfoEntityCopy.setEntityTag(incidentInfoEntity.getEntityTag());
            incidentInfoEntityCopy.setOldEntity(incidentInfoEntity);
            incidentInfoEntityCopy.setEditLink((incidentInfoEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            incidentInfoEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, incidentInfoEntityCopy);
    ...
    private boolean onSaveItem() {
        ...
        ((IncidentInfoSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(incidentInfoEntityCopy);
        } else {
            viewModel.update(incidentInfoEntityCopy);
        }
        return true;
    }

    /**
     * Create a new IncidentInfo instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new IncidentInfo instance
     */
    private IncidentInfo createIncidentInfo() {
        IncidentInfo incidentInfoEntity = new IncidentInfo(true);
        return incidentInfoEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<IncidentInfo> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(incidentInfoEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentIncidentinfosetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setIncidentInfo(incidentInfoEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isIncidentInfoValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_incidentinfo);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.incidentInfo.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<IncidentInfo> result) {
