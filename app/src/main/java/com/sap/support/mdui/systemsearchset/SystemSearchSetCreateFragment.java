...
import com.sap.support.databinding.FragmentSystemsearchsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing SystemSearch entity.
 * This fragment is contained in the {@link SystemSearchSetActivity}.
 */
public class SystemSearchSetCreateFragment extends InterfacedFragment<SystemSearch> {

    private static final Logger LOGGER = LoggerFactory.getLogger(SystemSearchSetCreateFragment.class);
    ...
    /** SystemSearch object and it's copy: the modifications are done on the copied object. */
    private SystemSearch systemSearchEntity;
    private SystemSearch systemSearchEntityCopy;

    /** DataBinding generated class */
    private FragmentSystemsearchsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** SystemSearch ViewModel */
    ...
    /**
     * This fragment is used for both update and create for SystemSearchSet to enter values for the properties.
     ...
     *            SystemSearch if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.systemSearch.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.systemSearch.getLocalName();
        }

        ((SystemSearchSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            systemSearchEntity = createSystemSearch();
        } else {
            systemSearchEntity = viewModel.getSelectedEntity().getValue();
        }

        SystemSearch workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (SystemSearch)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            systemSearchEntityCopy = (SystemSearch) systemSearchEntity.copy();
            systemSearchEntityCopy.setEntityTag(systemSearchEntity.getEntityTag());
            systemSearchEntityCopy.setOldEntity(systemSearchEntity);
            systemSearchEntityCopy.setEditLink((systemSearchEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            systemSearchEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, systemSearchEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isSystemSearchValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((SystemSearchSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(systemSearchEntityCopy);
        } else {
            viewModel.update(systemSearchEntityCopy);
        }
        return true;
    }

    /**
     * Create a new SystemSearch instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new SystemSearch instance
     */
    private SystemSearch createSystemSearch() {
        SystemSearch systemSearchEntity = new SystemSearch(true);
        return systemSearchEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<SystemSearch> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(systemSearchEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentSystemsearchsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setSystemSearch(systemSearchEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isSystemSearchValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_systemsearch);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.systemSearch.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<SystemSearch> result) {
