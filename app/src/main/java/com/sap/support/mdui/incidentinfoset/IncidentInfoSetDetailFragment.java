...
import com.sap.support.databinding.FragmentIncidentinfosetDetailBinding;
...
/**
 * A fragment representing a single IncidentInfo detail screen.
 * This fragment is contained in an IncidentInfoSetActivity.
 */
public class IncidentInfoSetDetailFragment extends InterfacedFragment<IncidentInfo> {

    /** Generated data binding class based on layout file */
    private FragmentIncidentinfosetDetailBinding binding;

    /** IncidentInfo entity to be displayed */
    private IncidentInfo incidentInfoEntity = null;
    ...
    /** Arguments: IncidentInfo for display */
    @Override
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel.getSelectedEntity().observe(this, entity -> {
            incidentInfoEntity = entity;
            binding.setIncidentInfo(entity);
            setupObjectHeader();
        });
    }
    ...
    private void onDeleteComplete(@NonNull OperationResult<IncidentInfo> result) {
        ...
        listener.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, incidentInfoEntity);
    }
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull IncidentInfo incidentInfoEntity) {
        if (EntityMediaResource.hasMediaResources(EntitySets.incidentInfoSet)) {
        ...
        } else if (incidentInfoEntity.getDataValue(IncidentInfo.incidentNumber) != null && !incidentInfoEntity.getDataValue(IncidentInfo.incidentNumber).toString().isEmpty()) {
            objectHeader.setDetailImageCharacter(incidentInfoEntity.getDataValue(IncidentInfo.incidentNumber).toString().substring(0, 1));
        } else {
    ...
    /**
     * Setup ObjectHeader with an instance of IncidentInfo
     */
    private void setupObjectHeader() {
        Toolbar secondToolbar = currentActivity.findViewById(R.id.secondaryToolbar);
        if (secondToolbar != null) {
            secondToolbar.setTitle(incidentInfoEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(incidentInfoEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(incidentInfoEntity));
            ...
            setDetailImage(objectHeader, incidentInfoEntity);
    ...
    private View setupDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = FragmentIncidentinfosetDetailBinding.inflate(inflater, container, false);
