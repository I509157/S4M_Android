...
import com.sap.support.databinding.FragmentNotesadvisorsetDetailBinding;
...
/**
 * A fragment representing a single NotesAdvisor detail screen.
 * This fragment is contained in an NotesAdvisorSetActivity.
 */
public class NotesAdvisorSetDetailFragment extends InterfacedFragment<NotesAdvisor> {

    /** Generated data binding class based on layout file */
    private FragmentNotesadvisorsetDetailBinding binding;

    /** NotesAdvisor entity to be displayed */
    private NotesAdvisor notesAdvisorEntity = null;
    ...
    /** Arguments: NotesAdvisor for display */
    @Override
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel.getSelectedEntity().observe(this, entity -> {
            notesAdvisorEntity = entity;
            binding.setNotesAdvisor(entity);
            setupObjectHeader();
        });
    }
    ...
    private void onDeleteComplete(@NonNull OperationResult<NotesAdvisor> result) {
        ...
        listener.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, notesAdvisorEntity);
    }
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull NotesAdvisor notesAdvisorEntity) {
        if (EntityMediaResource.hasMediaResources(EntitySets.notesAdvisorSet)) {
        ...
        } else if (notesAdvisorEntity.getDataValue(NotesAdvisor.pointer) != null && !notesAdvisorEntity.getDataValue(NotesAdvisor.pointer).toString().isEmpty()) {
            objectHeader.setDetailImageCharacter(notesAdvisorEntity.getDataValue(NotesAdvisor.pointer).toString().substring(0, 1));
        } else {
    ...
    /**
     * Setup ObjectHeader with an instance of NotesAdvisor
     */
    private void setupObjectHeader() {
        Toolbar secondToolbar = currentActivity.findViewById(R.id.secondaryToolbar);
        if (secondToolbar != null) {
            secondToolbar.setTitle(notesAdvisorEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(notesAdvisorEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(notesAdvisorEntity));
            ...
            setDetailImage(objectHeader, notesAdvisorEntity);
    ...
    private View setupDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = FragmentNotesadvisorsetDetailBinding.inflate(inflater, container, false);
