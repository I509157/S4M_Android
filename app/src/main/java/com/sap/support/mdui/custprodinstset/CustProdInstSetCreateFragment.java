...
import com.sap.support.databinding.FragmentCustprodinstsetCreateBinding;
...
/**
 * A fragment that presents a screen to either create or update an existing CustProdInst entity.
 * This fragment is contained in the {@link CustProdInstSetActivity}.
 */
public class CustProdInstSetCreateFragment extends InterfacedFragment<CustProdInst> {

    private static final Logger LOGGER = LoggerFactory.getLogger(CustProdInstSetCreateFragment.class);
    ...
    /** CustProdInst object and it's copy: the modifications are done on the copied object. */
    private CustProdInst custProdInstEntity;
    private CustProdInst custProdInstEntityCopy;

    /** DataBinding generated class */
    private FragmentCustprodinstsetCreateBinding binding;

    /** Indicate what operation to be performed */
    private String operation;

    /** CustProdInst ViewModel */
    ...
    /**
     * This fragment is used for both update and create for CustProdInstSet to enter values for the properties.
     ...
     *            CustProdInst if Operation is update
     */
    @Override
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (UIConstants.OP_CREATE.equals(operation)) {
            activityTitle = currentActivity.getResources().getString(R.string.title_create_fragment, EntityTypes.custProdInst.getLocalName());
        } else {
            activityTitle = currentActivity.getResources().getString(R.string.title_update_fragment) + " " + EntityTypes.custProdInst.getLocalName();
        }

        ((CustProdInstSetActivity)currentActivity).isNavigationDisabled = true;
        ...
        if(UIConstants.OP_CREATE.equals(operation)) {
            custProdInstEntity = createCustProdInst();
        } else {
            custProdInstEntity = viewModel.getSelectedEntity().getValue();
        }

        CustProdInst workingCopy = null;
        if( savedInstanceState != null ) {
            workingCopy =  (CustProdInst)savedInstanceState.getParcelable(KEY_WORKING_COPY);
        }
        if( workingCopy == null ) {
            custProdInstEntityCopy = (CustProdInst) custProdInstEntity.copy();
            custProdInstEntityCopy.setEntityTag(custProdInstEntity.getEntityTag());
            custProdInstEntityCopy.setOldEntity(custProdInstEntity);
            custProdInstEntityCopy.setEditLink((custProdInstEntity.getEditLink()));
        } else {
            //in this case, the old entity and entity tag should already been set.
            custProdInstEntityCopy = workingCopy;
        }
    }
    ...
    public void onSaveInstanceState(@NonNull Bundle outState) {
        outState.putParcelable(KEY_WORKING_COPY, custProdInstEntityCopy);
    ...
    private boolean onSaveItem() {
        if (!isCustProdInstValid()) {
            return false;
        }
        //set 'isNavigationDisabled' false here to make sure the logic in list is ok, and set it to true if update fails.
        ((CustProdInstSetActivity)currentActivity).isNavigationDisabled = false;
        if( progressBar != null ) progressBar.setVisibility(View.VISIBLE);
        if (operation.equals(UIConstants.OP_CREATE)) {
            viewModel.create(custProdInstEntityCopy);
        } else {
            viewModel.update(custProdInstEntityCopy);
        }
        return true;
    }

    /**
     * Create a new CustProdInst instance and initialize properties to its default values
     * Nullable property will remain null
     * @return new CustProdInst instance
     */
    private CustProdInst createCustProdInst() {
        CustProdInst custProdInstEntity = new CustProdInst(true);
        return custProdInstEntity;
    }

    /** Callback function to complete processing when updateResult or createResult events fired */
    private void onComplete(@NonNull OperationResult<CustProdInst> result) {
        ...
        if (result.getError() != null) {
            ...
            if( UIConstants.OP_UPDATE.equals(operation) && !isMasterDetail) {
                viewModel.setSelectedEntity(custProdInstEntityCopy);
            }
    ...
    private View setupDataBinding(@NonNull LayoutInflater inflater, ViewGroup container) {
        binding = FragmentCustprodinstsetCreateBinding.inflate(inflater, container, false);
        View rootView = binding.getRoot();
        binding.setCustProdInst(custProdInstEntityCopy);
        return rootView;
    }

    /** Validate the edited inputs */
    private boolean isCustProdInstValid() {
        LinearLayout linearLayout = getView().findViewById(R.id.create_update_custprodinst);
        boolean isValid = true;
        ...
        for (int i = 0; i < linearLayout.getChildCount(); i++) {
            ...
            Property property = EntityTypes.custProdInst.getProperty(propertyName);
     ...
     */
    private void handleError(@NonNull OperationResult<CustProdInst> result) {
