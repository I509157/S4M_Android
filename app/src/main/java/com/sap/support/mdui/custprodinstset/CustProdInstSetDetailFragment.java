...
import com.sap.support.databinding.FragmentCustprodinstsetDetailBinding;
...
/**
 * A fragment representing a single CustProdInst detail screen.
 * This fragment is contained in an CustProdInstSetActivity.
 */
public class CustProdInstSetDetailFragment extends InterfacedFragment<CustProdInst> {

    /** Generated data binding class based on layout file */
    private FragmentCustprodinstsetDetailBinding binding;

    /** CustProdInst entity to be displayed */
    private CustProdInst custProdInstEntity = null;
    ...
    /** Arguments: CustProdInst for display */
    @Override
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        viewModel.getSelectedEntity().observe(this, entity -> {
            custProdInstEntity = entity;
            binding.setCustProdInst(entity);
            setupObjectHeader();
        });
    }
    ...
    private void onDeleteComplete(@NonNull OperationResult<CustProdInst> result) {
        ...
        listener.onFragmentStateChange(UIConstants.EVENT_DELETION_COMPLETED, custProdInstEntity);
    }
     ...
     */
    private void setDetailImage(@NonNull ObjectHeader objectHeader, @NonNull CustProdInst custProdInstEntity) {
        if (EntityMediaResource.hasMediaResources(EntitySets.custProdInstSet)) {
        ...
        } else if (custProdInstEntity.getDataValue(CustProdInst.custNbr) != null && !custProdInstEntity.getDataValue(CustProdInst.custNbr).toString().isEmpty()) {
            objectHeader.setDetailImageCharacter(custProdInstEntity.getDataValue(CustProdInst.custNbr).toString().substring(0, 1));
        } else {
    ...
    /**
     * Setup ObjectHeader with an instance of CustProdInst
     */
    private void setupObjectHeader() {
        Toolbar secondToolbar = currentActivity.findViewById(R.id.secondaryToolbar);
        if (secondToolbar != null) {
            secondToolbar.setTitle(custProdInstEntity.getEntityType().getLocalName());
        } else {
            currentActivity.setTitle(custProdInstEntity.getEntityType().getLocalName());
        }
        ...
        if (objectHeader != null) {
            ...
            objectHeader.setSubheadline(EntityKeyUtil.getOptionalEntityKey(custProdInstEntity));
            ...
            setDetailImage(objectHeader, custProdInstEntity);
    ...
    private View setupDataBinding(LayoutInflater inflater, ViewGroup container) {
        binding = FragmentCustprodinstsetDetailBinding.inflate(inflater, container, false);
